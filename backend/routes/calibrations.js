...
const BASE_QUERY = `
  SELECT cr.*,
    e.equipment_name, e.model, e.serial_number, e.modules,
...
router.get('/', (req, res) => {
  try {
    const db = getDb();
    const records = db.prepare(BASE_QUERY + ' ORDER BY cr.calibration_date DESC').all();
    res.json(records);
  } catch (err) {
...
router.get('/due', (req, res) => {
  try {
    const db = getDb();
    const records = db.prepare(BASE_QUERY + `
...
router.post('/', (req, res) => {
  try {
    ...
    const {
      ...
      notes, service_type, fee, currency
    } = req.body;
